    ...
    try {
      ...
      const { data, error } = await authenticatedFetchJson('/api/admin/groups', {
        ...
        body: JSON.stringify({
          ...
          courseDeadline: createGroupFormData.courseDeadline || null
        }),
      });
      ...
      // Reset form and refresh data
      setCreateGroupFormData({ name: '', organizationId: selectedOrganization?.id || '', courseDeadline: '' });
    ...
    try {
      ...
      const { data, error } = await authenticatedFetchJson(`/api/admin/groups/${editingGroup.id}`, {
        ...
        body: JSON.stringify({
          ...
          courseDeadline: editingGroup.courseDeadline || null
        }),
      });
              ...
              <form onSubmit={handleCreateGroup} className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">
                    תאריך יעד לסיום הקורס <span className="text-slate-400">(אופציונלי)</span>
                  </label>
                  <input
                    type="date"
                    value={createGroupFormData.courseDeadline}
                    onChange={(e) => setCreateGroupFormData(prev => ({ ...prev, courseDeadline: e.target.value }))}
                        ...
                        {editingOrganization?.id === org.id ? (
                          ...
                          <>
                            ...
                            <div className="flex items-center gap-2 text-xs text-slate-500">
                              ...
                              <span>ID: {org.id}</span>
                              <button
                                onClick={(e) => handleCopyId(org.id, e)}
                ...
                {filteredGroups.map(group => (
                  <div key={group.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        {editingGroup?.id === group.id ? (
                          <form onSubmit={handleEditGroup} className="space-y-2">
                            <div className="flex items-center gap-2">
                              <input
                                type="text"
                            ...
                            <div className="flex items-center gap-2">
                              <label className="text-xs text-slate-500 whitespace-nowrap">תאריך יעד לסיום הקורס:</label>
                              <input
                                type="date"
                                value={editingGroup.courseDeadline || ''}
                                onChange={(e) => setEditingGroup(prev => prev ? { ...prev, courseDeadline: e.target.value } : null)}
                                className="px-2 py-1 border border-slate-200 rounded text-xs focus:outline-none focus:ring-2 focus:ring-green-500"
                              />
                              {editingGroup.courseDeadline && (
                                <button
                                  type="button"
                                  onClick={() => setEditingGroup(prev => prev ? { ...prev, courseDeadline: '' } : null)}
                          ...
                          <>
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                              ...
                              <button
                                onClick={(e) => handleCopyId(group.id, e)}
                                className="inline-flex items-center text-slate-400 hover:text-slate-600 transition-colors"
                                title="העתק ID"
                              >
                                {copiedId === group.id ? (
                    ...
                    {editingGroup?.id !== group.id && (
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => setEditingGroup({ id: group.id, name: group.name, courseDeadline: group.course_deadline ? new Date(group.course_deadline).toISOString().split('T')[0] : '' })}
                          className="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-700 rounded text-xs font-medium transition-colors"
                        >
                          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
