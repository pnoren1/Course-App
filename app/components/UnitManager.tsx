      ...
      if (editingUnit) {
        ...
        const { error } = await rlsSupabase.raw
          ...
          .update({
            ...
            order: formData.order_number, // Use 'order' field for your DB
        ...
        const { error } = await rlsSupabase.raw
          ...
          .insert([{
            ...
            description: formData.description.trim() || null,
            order: formData.order_number // Use 'order' field for your DB
          } as any]); // Use type assertion to handle schema mismatch

        if (error) {
          console.error('Error creating unit:', error);
          alert('שגיאה ביצירת היחידה');
          return;
        }
      }
