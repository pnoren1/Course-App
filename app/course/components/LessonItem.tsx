...
export default function LessonItem({
  ...
  return (
    <li key={lesson.id} className={`p-6 transition-all duration-200 ${
      isOpen ? "bg-white/90 backdrop-blur-sm" : "hover:bg-white/60"
    } ${lesson.is_lab ? "border-l-4 border-l-blue-300/60" : ""}`}>
      <div className="flex items-center justify-between gap-4">
        ...
        >
          <div
            className={`flex items-center justify-center w-12 h-12 rounded-xl text-sm font-semibold transition-all duration-200 border ${
              isLocked 
                ? "bg-slate-100/80 text-slate-400 border-slate-200/60" 
                : isOpen 
                  ? lesson.is_lab 
                    ? "bg-gradient-to-br from-blue-100 to-blue-50 text-blue-700 border-blue-200/60" 
                    : "bg-gradient-to-br from-indigo-100 to-indigo-50 text-indigo-700 border-indigo-200/60"
                  : lesson.is_lab
                    ? "bg-gradient-to-br from-blue-50 to-slate-50 text-blue-600 border-blue-200/60"
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              {lesson.is_lab && (
                <span className="inline-flex items-center gap-1 px-2 py-1 bg-gradient-to-r from-blue-100 to-blue-50 text-blue-700 border border-blue-200/60 rounded-md text-xs font-semibold">
            ...
            <div className="flex items-center gap-4 text-sm text-slate-500">
              ...
              {isOpen && (
                <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-medium border ${
                  lesson.is_lab 
                    ? "bg-gradient-to-r from-blue-50 to-blue-25 text-blue-700 border-blue-200/60" 
                    : "bg-gradient-to-r from-indigo-50 to-indigo-25 text-indigo-700 border-indigo-200/60"
                }`}>
        ...
        <div className="flex items-center gap-3">
          {isLocked ? (
            <div className="inline-flex items-center gap-2 text-xs px-3 py-2 rounded-xl bg-slate-100/80 border border-slate-200/60 text-slate-500 font-medium">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              className={`inline-flex items-center justify-center w-10 h-10 rounded-xl transition-all duration-200 border ${
                isOpen 
                  ? lesson.is_lab
                    ? "bg-gradient-to-br from-blue-100 to-blue-50 text-blue-600 rotate-90 border-blue-200/60"
                    : "bg-gradient-to-br from-indigo-100 to-indigo-50 text-indigo-600 rotate-90 border-indigo-200/60"
