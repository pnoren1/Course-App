...
export default function LessonItem({
  ...
  return (
    <li key={lesson.id} className={`p-6 transition-all duration-200 ${
    ...
    } ${lesson.is_lab ? "border-l-4 border-blue-200" : ""}`}>
      <div className="flex items-center justify-between gap-4">
        ...
        >
          <div
            className={`flex items-center justify-center w-12 h-12 rounded-xl text-sm font-semibold transition-all duration-200 ${
              isLocked 
        ...
        <div className="flex items-center gap-3">
          {isLocked ? (
            <div className="inline-flex items-center gap-2 text-xs px-3 py-2 rounded-xl bg-slate-100 text-slate-500 font-medium">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
