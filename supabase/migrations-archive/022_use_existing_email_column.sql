-- Fix functions to use existing 'email' column instead of 'user_email'

-- Drop existing functions first to avoid conflicts
DROP FUNCTION IF EXISTS get_user_roles_with_org();
DROP FUNCTION IF EXISTS search_user_profiles(TEXT);
DROP FUNCTION IF EXISTS get_user_profile_by_email(TEXT);

-- Update get_user_roles_with_org function to use existing email column
...
-- Update search function to use existing email column
...
-- Update get_user_profile_by_email function
...
-- Update assign_user_role_with_org function
...
BEGIN
    ...
    DO UPDATE SET 
        ...
        email = target_user_email,  -- Use existing email column
...
-- Update trigger function to use existing email column
...
-- Create index on existing email column if it doesn't exist
CREATE INDEX IF NOT EXISTS idx_user_profile_email ON user_profile(email);

-- Update existing records to populate email if empty
UPDATE user_profile 
SET email = au.email
FROM auth.users au
...
-- Add comment
COMMENT ON COLUMN user_profile.email IS 'User email address from auth.users for easy identification and search';
